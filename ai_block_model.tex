...
C_\Pi : & [0,1]^d & \longrightarrow & [0,1] \\
	...
	Let $\bar{O}$ the unique maximal element of $\mathcal{L}(\textbf{X})$ and consider $\hat{C}_1 \subset \bar{O}_1$. Consider now $j \in \{1,\dots,d\} \setminus \hat{O}_1$ and we to know if $j \in \bar{O}_1$, again, using knowledge of the Pickands dependence function, the statistician can evaluate the following value
