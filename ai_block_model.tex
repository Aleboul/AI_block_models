...
\end{algorithm}
	...
	\label{prop:concentration_inequality}
		For $t >0$, we have
		\begin{equation*}
			\mathbb{P}\left\{ |\hat{\theta}_n - \theta | \geq t \right\} \leq 16 d^2 \exp\left\{ - \frac{nt^2}{8}\right\}.
		\end{equation*}
	\end{proposition}
...
\end{algorithm}
	...
	\begin{corollary}
		...
		\begin{equation*}
		\alpha_s \geq 2 \sqrt{\frac{6(1+\gamma)}{n} \ln\left(d s^{\frac{2}{3(1+\gamma)}}\right)}, \quad \eta_{s} \geq 2 \sqrt{\frac{6(1+\gamma)}{n} \ln\left(d s^{\frac{2}{3(1+\gamma)}}\right)} + \alpha_s,
		\end{equation*}
...
\phi_k & : & \ell^{\infty}(\Delta_{d-1}) & \to & \ell^{\infty}(S_{d}) \\
 & & x & \mapsto & x(\textbf{0}, t_{i_{k,1}},\dots, t_{i_{k,d_k}}, \textbf{0}). \\
	...
	\begin{proof}[Proof of Corollary \ref{corr:exact_recovery}]
		...
		\begin{equation*}
			\mathbb{P}\left\{|\hat{\theta}_n^{(U)} - \theta^{(U)}| \geq t \right\} \leq 16 s^2 \exp \left\{-\frac{nt^2}{8} \right\},
		\end{equation*}
		where $|U| = s$. By considering $\delta \in ]0,1[$ and solve the following equation
		\begin{equation*}
			\frac{\delta}{d^3} = 16 s^2 \exp \left\{-\frac{nt^2}{8} \right\},
		\end{equation*}
		with respect to $t$ gives that :
		\begin{equation*}
			\mathbb{P}\left\{ \bigcup_{U \in ALG} |\hat{\theta}_n^{(U)} - \theta^{(U)}| \geq 2 \sqrt{\frac{2}{n} \ln \left( \frac{16 s^2 d^3}{\delta} \right)} \right\} \leq \delta.
		\end{equation*}
		Now, taking $\delta = 16d^{-3\gamma}$, with $\gamma > 0$, we have that for every $U \in ALG$
		\begin{equation*}
			|\hat{\theta}_n^{(U)} - \theta^{(U)}| \leq 2 \sqrt{\frac{6(1+\gamma)}{n} \ln \left(s^{\frac{2}{3(1+\gamma)}} d \right)},
		\end{equation*}
		with probability higher than $1 - 16d^{-3\gamma}$. The result then follows from Proposition \ref{prop:exact_recovery}, since for every $s \in \{1, \dots, d-1\}$, $\tau_{s} \leq 8 s \sqrt{\frac{6(1+\gamma)}{n} \ln \left(s^{\frac{2}{3(1+\gamma)}} d \right)}$ with probability higher than $1-16d^{-3\gamma}$.
	\end{proof}
