...
\usepackage{amssymb, setspace, graphicx,stmaryrd, dsfont, amsmath, amsfonts, tikz, amsthm, algorithm,algcompatible,hyperref,subcaption, enumitem}
...
\begin{algorithmic}[1]
\Procedure{Clust}{$S$, $\alpha$, $\hat{\mathcal{X}}$}
    ...
    \While{$S \neq \emptyset$}
    		...
    		\If{$|S| > 1 $}
    			...
    			\If{$\hat{\chi}(a_l,b_l) < \alpha$}
    				\State $\hat{O}_l = \{a_l\}$
    			\EndIf
    			\If{$\hat{\chi}(a_l,b_l) \geq \alpha$}
 ...
 & & x & \mapsto & x(\textbf{0}, t_{i_{k,1}},\dots, t_{i_{k,d_k}}, \textbf{0}). \\
		...
		\centering
			\begin{tikzpicture}[node distance=2cm, auto]
				(0,0) \node(A) {$\sqrt{n}\left(\hat{A}_{n,m} - A\right)$};
				(5,0)\node(B) [right of=A, xshift = 1cm ] {$\mathcal{E}_{nK}$};
				\node (C) [below of=B] {$\left(\sqrt{n}\left(\hat{A}_{n,m} - A\right) ; w_1\sqrt{n}\left(\hat{A}_{n,m}^{(O_1)} - A^{(O_1)}\right), \dots, w_k(\textbf{t})\sqrt{n}\left(\hat{A}_{n,m}^{(O_k)} - A^{(O_k)}\right)\right)$};	
				\draw[->](A) to node {}(B);
				\draw[->](A) to node [left] {$\phi$}(C);
				\draw[->](C) to node [below right=0.5ex] {$\psi$}(B);
			\end{tikzpicture}
